...
class MlpModule(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.propriate_input_dim = propriate_input_dim
        ...
        self.propriate_mlp = EncoderModel(self.propriate_input_dim, self.latent_dim, 512, 1, activation)
        print(f"Propriocetive Generator MLP: {self.propriate_mlp}")
    ...
    def propriate_mlp_forward(self, input):
        latent = self.propriate_mlp(input)
        if self.output_normalize != 0:
            return torch.nn.functional.normalize(latent, p=2, dim=-1) * self.output_normalize
        else:
            return latent
