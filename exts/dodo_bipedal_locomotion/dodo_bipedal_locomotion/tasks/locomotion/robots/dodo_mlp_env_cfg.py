"""
Dodo Robot Environment Configurations for Flat Terrain Environments 
The different configurations from Base Environment: 
    1. self.curriculum.terrain_levels = None 

The different configurations from Play Environment:        
...
@configclass
class DodoMlpBaseEnvCfg(FlatEnvCfg):
...
@configclass
class DodoMlpBaseEnvCfg_PLAY(DodoMlpBaseEnvCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class DodoBlindFlatEnvCfg(DodoMlpBaseEnvCfg):
    def __post_init__(self):
        super().__post_init__()
...
@configclass
class DodoBlindFlatEnvCfg_PLAY(DodoMlpBaseEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
